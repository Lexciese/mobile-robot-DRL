...
class SIM(SIM_ENV):
    ...
    def reset(
        ...
        # Try up to 100 times to find a collision-free spawn    
        max_attempts = 100
        attempt = 0
        collision = True
        ...
        while collision and attempt < max_attempts:
            ...
            # Sample random state if not provided
            if robot_state is None:
                robot_state = [
                    random.uniform(1, 9),  # x-coordinate
                    random.uniform(1, 9),  # y-coordinate
                    random.uniform(-np.pi, np.pi),  # orientation
                ]
            ...
            # Simulate a step with zero velocity to update the collision state
            self.env.step(action_id=0, action=np.array([[0.0], [0.0]]))
            collision = self.env.robot.collision

            if collision:
                robot_state = None  # Resample if in collision
